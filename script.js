(() => {
  ...
  const initRequestForm = () => {
    ...
    const extraServicesList = $('#extra-services-list');
    ...
    const createExtraServiceItem = () => {
      ...
      item.append(select, removeBtn);
      extraServicesList.appendChild(item);
    };

    if (addServiceBtn) addServiceBtn.addEventListener('click', createExtraServiceItem);
    ...
    form.addEventListener('submit', (event) => {
      ...
      data.getAll('extraServices[]').forEach((item) => {
