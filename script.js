...
const reviewsViewport = reviewsTrack ? reviewsTrack.parentElement : null;
...
if (reviewsTrack && reviewsSlider) {
  ...
  const autoplayDelayMs = 10000;

  const updateViewportHeight = () => {
    if (!reviewsViewport || !slides[current]) return;
    reviewsViewport.style.height = `${slides[current].offsetHeight}px`;
  };
  ...
  slides.forEach((slide) => {
    ...
    controls.className = 'review-card-controls';

    const prevBtn = document.createElement('button');
    prevBtn.type = 'button';
    prevBtn.className = 'review-card-arrow review-card-arrow--prev';
    prevBtn.setAttribute('aria-label', 'Предыдущий отзыв');
    prevBtn.textContent = '←';

    const nextBtn = document.createElement('button');
    nextBtn.type = 'button';
    nextBtn.className = 'review-card-arrow review-card-arrow--next';
  ...
  reviewsSlider.addEventListener('mouseleave', restartAutoplay);
  reviewsSlider.addEventListener('focusin', stopAutoplay);
  reviewsSlider.addEventListener('focusout', restartAutoplay);
